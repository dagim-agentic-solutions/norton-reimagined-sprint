...
## UI Design
- **Scorecard panel:** lives between the concept preview and the download buttons. Layout:
  - Section heading (“Score this concept”) + subtle “Saved locally” / “Not scored yet” status.
  - Three slider rows with labels, helper text (1 = weak, 5 = strong), and value badge.
...
## Data & Interaction
- Scores live exclusively in `sessionStorage` (`conceptScores:{conceptId}` → `{ wanted, understood, solves }`). Each slider update writes immediately.
- Summarize iterates over the currently rendered cards, filters to the ones with stored votes, computes `avg = (wanted + understood + solves) / 3`, sorts descending, and renders the leaderboard.
...
- If Summarize is invoked with zero scored concepts, display an inline warning (“Score at least one concept before summarizing”).
