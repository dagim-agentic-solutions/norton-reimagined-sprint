...
export async function onRequest(context) {
  ...
  // Parse path segments after /api/idea-grouping
  const base = '/api/idea-grouping';
  const rest = path.slice(base.length); // '', '/idea', '/idea/ID', '/column/ID', '/lock', '/unlock'
  const parts = rest.split('/').filter(Boolean); // ['idea'], ['idea','ID'], etc.
